...
def workflow_complete():
    ...
    try:
        ...
        step1_result = massive_data_collector.collect_comprehensive_data(
            produto=data.get('produto', ''),
            nicho=data.get('nicho', ''),
            publico=data.get('publico', ''),
            session_id=session_id
        ...
        viral_analysis = asyncio.run(viral_content_analyzer.analyze_and_capture_viral_content(
            search_results=step1_result,
            session_id=session_id
        ))
        
        # Etapa 2.5: Síntese com IA
        from services.enhanced_synthesis_engine import enhanced_synthesis_engine
        
        step2_result = asyncio.run(enhanced_synthesis_engine.execute_enhanced_synthesis(
            session_id=session_id
        ))
        ...
        # Gera todos os módulos
        modules_result = asyncio.run(enhanced_module_processor.generate_all_modules(session_id))
