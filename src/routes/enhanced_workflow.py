...
async def _capture_viral_social_media_images(query: str, context: str, session_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        async with ViralContentAnalyzer() as viral_analyzer:
            ...
            # 🎯 ADICIONA DOWNLOAD DE IMAGENS PÚBLICAS
            try:
                logger.info("🖼️ BAIXANDO IMAGENS PÚBLICAS ADICIONAIS...")
                from services.public_image_downloader import PublicImageDownloader
                
                async with PublicImageDownloader() as downloader:
                    public_images = await downloader.download_public_images(query, session_id, limit=15)
                    logger.info(f"✅ {len(public_images)} IMAGENS PÚBLICAS BAIXADAS!")
                    
                    # Atualiza o resumo
                    viral_summary["public_images_downloaded"] = len(public_images)
                    viral_summary["total_images"] = viral_summary['screenshots_captured'] + len(public_images)
                    
            except Exception as e:
                logger.error(f"❌ Erro ao baixar imagens públicas: {e}")
