...
# from .mcp_supadata_manager import MCPSupadataManager  # Comentado - não existe
...
class CompetitorContentCollector:
    def __init__(self):
        # self.mcp_supadata_manager = MCPSupadataManager()  # Comentado - não existe
    ...
    def collect_and_analyze_content(self, competitor_name: str) -> List[Dict[str, Any]]:
        ...
        for base_url in config["base_urls"]:
            ...
            for url in new_urls:
                ...
                # extracted_data = self.mcp_supadata_manager.extract_from_url(url)  # Comentado
                # Simulação de extração para manter funcionalidade
                extracted_data = {
                    "extracted_text": f"Conteúdo simulado extraído de {url}",
                    "title": f"Título da página {url}"
                }
                if "error" not in extracted_data:
                    ...
                    logger.warning(f"Falha ao extrair conteúdo de {url}: dados simulados")
